...
const LunchSingle = ({route, navigation}) => {
    ...
    switch (mealType) {
      case 'lunch':
        try {
          ...
          console.log('morngsnack item added successfully');
          console.log(mealDetails, 'dark');
          // Handle any post-addition logic or navigation here
          setIsLoading(false);
        } catch (error) {
          setIsLoading(false);
          console.error('Error adding morng item:', error);
  ...
  const handleDelete = (itemIndex: number, mealType: string) => {
    switch (mealType) {
      ...
      case 'lunch':
        const newMorningSnackItems = [...lunchItems];
  ...
  // 0 + 1 + 2 + 3 + 4
  const totallunchItemsCalorie = lunchItems.reduce(
  ...
  console.log(totallunchItemsCalorie, 'total sum');

  // Format the number to always show 2 decimal places
  const formattedTotalCalorie = totallunchItemsCalorie.toFixed(2);

  console.log('Total morngSnack calorie:', formattedTotalCalorie);
      ...
      <Block
        ...
        <CircularProgress
          ...
          circleBackgroundColor={
            totallunchItemsCalorie > data.calories * 0.2
              ? 'lightcoral'
              : '#353353'
          }
          title={
